...
function install() {
    ...
    ip6tables -t nat -A POSTROUTING -o ${SERVER_PUBLIC_NIC} -j MASQUERADE

    ufw allow 443/tcp >/dev/null 2>&1
    ufw allow 3478/udp >/dev/null 2>&1
    ufw allow 41461/udp >/dev/null 2>&1
...
function remove() {
    if [ -f "/usr/bin/tailscale" ]; then
        ...
        ip6tables -t nat -D POSTROUTING -o ${SERVER_PUBLIC_NIC} -j MASQUERADE

        ufw delete allow 443/tcp >/dev/null 2>&1
        ufw delete allow 3478/udp >/dev/null 2>&1
        ufw delete allow 41461/udp >/dev/null 2>&1
        ...
        apt-get remove tailscale -y >/dev/null 2>&1
